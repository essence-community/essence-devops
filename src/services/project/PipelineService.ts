...
export class PipelineService {
    ...
    async findAll(json: JsonBody, user = '999999', req: Request): Promise<[PipelineModel[], number]> {
        ...
        await Promise.all(
            rows.map(async (val) => {
                (val as any).history = await this.connection
                    .createQueryBuilder()
                    .select('history')
                    ...
                    .limit(1)
                    .getOne();
                (val as any).last_success = await this.connection
                    .createQueryBuilder()
        ...
        if (data.parameter) {
            const parameter: ParameterModel & {
                ...
                .innerJoinAndMapOne(
                    ...
                    DServerTypeModel,
                    'serverType',
                    'server.ck_d_server_type = serverType.ck_id',
                )
                .where({
