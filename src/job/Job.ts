...
export class Job {
    ...
    async execute() {
        ...
        const res = await queryRunner.manager
            ...
            .innerJoinAndMapOne('history.parameter', ParameterModel, 'parameter', 'parameter.ck_id = pipeline.parameter.ck_id')
            ...
            .getMany() as IJobData[];
        try {
            await Promise.all(res.map((val) => rep.save({
                ...val,
                status: {
                    ck_id: 'processing'
                }
        ...
        try {
            await Promise.all(res.map((val) => new Promise(async (resolve, reject) => {
                ...
                try {
                    ...
                    if (parameter.ansible_ssh_private_key_file) {
                        if (parameter.ansible_ssh_private_key_file.startsWith('/')) {
                            ...
                            fs.writeFileSync(path.resolve(ansible_path, 'id_key'), parameter.ansible_ssh_private_key_file);
                            fs.chmodSync(path.resolve(ansible_path, 'id_key'), '600');
                            hosts+= ` ansible_ssh_private_key_file=${path.resolve(ansible_path, 'id_key')}`;
                        }
                    }
                    fs.writeFileSync(path.join(ansible_path, 'hosts'), hosts);
                    const fn = builders[val.parameter.server.dServerTypeModel.ck_id];
                    if (fn) {
                        fs.writeFileSync(path.join(ansible_path, 'deploy.yaml'), fn(ansible_path, val, parameter, env));
                    } else {
                        throw new Error(`Not found server type ${val.parameter.server.dServerTypeModel.ck_id}`);
                    }
                    const res = await new Promise<boolean>((resolve, reject) => {
                        exec('ansible-playbook -i hosts deploy.yaml -vvvv', {
                            cwd: ansible_path,
                            ...
                            if (err || stdout.indexOf('failed=0') === -1) {
                                ...
                                }).then(() => resolve(false), (err) => {
                                    log.error('ANSIBLE STDOUT:\n%s\nSTDERR:\n%s', stdout, stderr, err);
                                    return reject(err);
                                });
                            }
