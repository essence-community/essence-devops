...
const dockerBuildRegistry = (dir: string, data: IJobData, parameter: Record<string, string>, env: Record<string, string>): string => {
    const yaml = [];
        ...
        tasks: [
            {
                ...
                docker_image: {
                    path: data.pipeline.cv_path_extra ? path.join(parameter.build_dir || '/tmp', data.ck_id, data.pipeline.cv_path_extra) : path.join(parameter.build_dir || '/tmp', data.ck_id),
...
const podmanBuildRegistry = (dir: string, data: IJobData, parameter: Record<string, string>, env: Record<string, string>): string => {
    const yaml = [];
    const [name, tag] = data.pipeline.cv_name_image.split(':');
    const names = name.split('/');
    yaml.push({
        ...
        tasks: [
            {
                ...
                'containers.podman.podman_image': {
                    ...
                    name: names.length === 2 ? name : path.join(names[names.length-2], names[names.length-1]),
                    build: {
                        extra_args: Object.entries(env).map(([key,value]) => `--build-arg=${key}=${value}`).join(' ')
                    },
                    nocache: 'yes',
                    force: 'yes',
                    push: 'yes',
...
const appPodmanBuild = (dir: string, data: IJobData, parameter: Record<string, string>, env: Record<string, string>): string => {
    const yaml = [];
    yaml.push({
        hosts: 'app-hosts',
        tasks: [
...
const appDockerBuild = (dir: string, data: IJobData, parameter: Record<string, string>, env: Record<string, string>): string => {
    const yaml = [];
    yaml.push({
        hosts: 'app-hosts',
        tasks: [
